...
class EditorImagen:
    def __init__(self, master, image_path, callback_guardado, modo_video=False):
        ...
        Label(header, text="Arrastra para seleccionar el área a conservar (Presiona ENTER para aplicar)",
        ...
        # NUEVO: Evento del teclado para ENTER
    ...
    # NUEVO: Método para procesar el ENTER
    def on_enter(self, event):
        # Solo ejecuta el guardado si el botón está activo (es decir, hay un recorte válido)
        if self.btn_guardar['state'] == 'normal':
            self.guardar()
